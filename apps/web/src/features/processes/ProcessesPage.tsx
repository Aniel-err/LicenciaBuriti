...
function ProcessDrawer({ process, onClose }: { process: Processo; onClose(): void }) {
  const { state, session, refresh } = useApp();
  ...
  const perform = async (name: string, action: () => Promise<unknown>, shouldRefresh = true) => {
    ...
    try {
      ...
      setOperationError(getUserErrorMessage(cause));
    } finally {
      setBusyAction("");
    }
  };
  ...
  return <Modal title={`Processo ${process.numero}`} onClose={onClose}>
    ...
    <section className="detail-content">
      ...
      {tab === "documentos" ? <>
        ...
        {isInternal && session ? <form className="message-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const name = String(form.get("documentName") ?? ""); void perform("request-document", () => requestProcessDocumentApi(session.token, process.id, name)); event.currentTarget.reset(); }}><label>Solicitar documento complementar<input name="documentName" minLength={3} required /></label><Button loading={busyAction === "request-document"}><FilePlus2 />Solicitar complemento</Button></form> : null}
      </> : null}
...
function NewProcessWizard({ onClose }: { onClose(): void }) {
  ...
  return <Modal title="Nova solicitação" onClose={onClose}>
    ...
    <div className="wizard-body">
      ...
      {step === 5 ? <div className="success-state"><strong>Solicitação protocolada</strong><span>{created}</span><div><Button variant="secondary" onClick={() => navigator.clipboard.writeText(created)}><Copy />Copiar</Button><Button variant="secondary" onClick={() => window.print()}><Printer />Imprimir</Button></div></div> : null}
    </div>
